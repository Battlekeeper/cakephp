...
class ControllerFactoryTest extends TestCase
{
     ...
     *  2. The previous controller and all connected objects (ComponentRegistry,
     *     EventManager, etc.) are GC-eligible once the next request's create()
     *     drops the container's ComponentRegistry entry.
     */
    public function testControllerReleasedBetweenRequestsInWorkerMode(): void
    {
        // --- Request N ---
        $request = new ServerRequest([
            'url' => 'cakes',
        ...
        // Drop the caller's strong reference (mirrors worker-loop behaviour).
        unset($controller);

        // After invoke() the factory must not retain its own reference.
        // We cannot observe $this->controller directly, but starting request N+1
        // via create() will also drop the container's ComponentRegistry entry,
        // leaving only the GC-collectable cycle.
        ...
        $this->factory->create($request2);

        // Collect any reference cycles (Controller↔ComponentRegistry↔EventManager).
        gc_collect_cycles();

        $this->assertNull(
            $weak->get(),
            'Controller from request N should be GC-eligible once request N+1 starts.',
        );
    }
}
